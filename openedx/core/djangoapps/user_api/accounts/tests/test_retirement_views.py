...
class TestPartnerReportingList(ModuleStoreTestCase):
    ...
    def assert_status_and_user_list(self, expected_users, expected_status=status.HTTP_200_OK):
        ...
        # Due to the nested list of orgs we can't use assertCountEqual for the whole thing,
        # this works around that
        found = False
        for testing_user in expected_users:
            for returned_user in returned_users:
                if returned_user['original_username'] == testing_user['original_username']:
                    found = True
                    # Check the orgs sub-list
                    self.assertCountEqual(returned_user['orgs'], testing_user['orgs'])

                    # Remove the orgs so we can compare the rest of the structure
                    del returned_user['orgs']
                    del testing_user['orgs']
                    self.assertCountEqual(returned_user, testing_user)

                    # Remove this user from the list, when we're done it should be empty
                    returned_users.remove(returned_user)
                    break
            # Check that each expected user was found in the returned list
            self.assertTrue(found)
        # Check that there are no extra users returned that weren't expected
        self.assertFalse(returned_users)
