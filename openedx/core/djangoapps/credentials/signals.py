...
def send_grade_if_interesting(user, course_run_key, mode, status, letter_grade, percent_grade):
    ...
    if mode is None or status is None:
        try:
            cert = GeneratedCertificate.objects.get(user=user, course_id=course_run_key)
            mode = cert.mode
            status = cert.status
        except GeneratedCertificate.DoesNotExist:
            return

    # We only care about grades for which there is a certificate. Don't worry about whether it's available as well as
    # awarded. Just awarded is good enough to record a verified attempt at a course.
    # We want even the grades that didn't pass the class because Credentials wants to know about those too.
...
def handle_grade_change(sender, user, course_grade, course_key, deadline, **kwargs):
    """
...
def handle_cert_change(sender, user, course_key, mode, status, **kwargs):
    """
