    ...
    def _collect_one_click_purchase_eligibility_data(self):
        ...
        if skus:
            try:
                ...
                # The program price from ecommerce can be calculated more quickly if we fetch
                # the anonymous user price. To get the full program price specific to a user,
                # we would send the "&username=" query parameter instead. Note that user
                # specific program prices is very slow.
                # See LEARNER-5555 for more details.
